package key

import (
    "fmt"
    "errors"
    "crypto/rsa"
    "crypto/rand"
    "crypto/x509"
    "crypto/x509/pkix"
    "encoding/pem"
    "encoding/asn1"
    "encoding/base64"

    "github.com/deatil/lakego-filesystem/filesystem"

    cryptobin_rsa "github.com/deatil/go-cryptobin/cryptobin/rsa"
    cryptobin_pkcs12 "github.com/deatil/go-cryptobin/pkcs12"
)

type Errs struct{
    Err1 string
    Err2 string
    Err3 string
}

func TestPfx() []Errs {
    var errs = make([]Errs, 0)

    for commonName, base64P12 := range testdata {
        newErr := Errs{}

        p12, _ := base64.StdEncoding.DecodeString(base64P12)

        priv, cert, err := cryptobin_pkcs12.Decode(p12, "")
        if err != nil {
            newErr.Err1 = err.Error()
        } else {
            if err := priv.(*rsa.PrivateKey).Validate(); err != nil {
                newErr.Err2 = fmt.Errorf("error while validating private key: %v", err).Error()
            }

            if cert.Subject.CommonName != commonName {
                newErr.Err3 = fmt.Errorf("expected common name to be %q, but found %q", commonName, cert.Subject.CommonName).Error()
            }
        }

        if newErr.Err1 == "" &&
            newErr.Err2 == "" &&
            newErr.Err3 == "" {
            fmt.Println(commonName)
            fmt.Printf("%#v", priv)
        }

        errs = append(errs, newErr)
    }

    // certBytes, err = x509.CreateCertificate(rand.Reader, newCert, newCa, this.publicKey, this.privateKey)

    return errs
}

func MakePKCS12() error {
    fs := filesystem.New()

    path := "./runtime/key/pkcs12/ca/%s"

    caCertFile := fmt.Sprintf(path, "caCert.cer")
    certificateFile := fmt.Sprintf(path, "certificate.cer")
    privateKeyFile := fmt.Sprintf(path, "private.key")

    caCertData, _ := fs.Get(caCertFile)
    certificateData, _ := fs.Get(certificateFile)
    privateKeyData, _ := fs.Get(privateKeyFile)

    // Parse PEM block
    var caCertBlock *pem.Block
    if caCertBlock, _ = pem.Decode([]byte(caCertData)); caCertBlock == nil {
        return errors.New("caCert err")
    }

    caCerts, err := x509.ParseCertificates(caCertBlock.Bytes)
    if err != nil {
        return err
    }

    // Parse PEM block
    var certificateBlock *pem.Block
    if certificateBlock, _ = pem.Decode([]byte(certificateData)); certificateBlock == nil {
        return errors.New("certificate err")
    }

    certificates, err := x509.ParseCertificates(certificateBlock.Bytes)
    if err != nil {
        return err
    }

    privateKey := cryptobin_rsa.NewRsa().
        FromPrivateKey([]byte(privateKeyData)).
        GetPrivateKey()

    // pfxData, err := cryptobin_pkcs12.Encode(rand.Reader, privateKey, certificates[0], caCerts, "123")
    pfxData, err := cryptobin_pkcs12.Encode(rand.Reader, privateKey, certificates[0], caCerts, "", cryptobin_pkcs12.Opts{
        PKCS8Cipher: cryptobin_pkcs12.GetPKCS8PbeCipherFromName("SHA1AndRC2_40"),
        Cipher: cryptobin_pkcs12.CipherSHA1AndRC2_40,
        KDFOpts: cryptobin_pkcs12.MacOpts{
            SaltSize: 8,
            IterationCount: 1,
            HMACHash: cryptobin_pkcs12.SHA1,
        },
    })
    if err != nil {
        return err
    }

    pkcs12File := fmt.Sprintf(path, "pkcs12.p12")
    fs.Put(pkcs12File, string(pfxData))

    // 保存为 pem 证书
    pkcs12Blocks, err := cryptobin_pkcs12.ToPEM(pfxData, "")
    if err != nil {
        return err
    }

    for k, block := range pkcs12Blocks {
        pkcs12BlockKeyData := pem.EncodeToMemory(block)

        pkcs12BlockFile := fmt.Sprintf(path, fmt.Sprintf("pkcs12-block-%d.key", k))
        fs.Put(pkcs12BlockFile, string(pkcs12BlockKeyData))
    }

    // EncodeTrustStore
    pfxDataTrustStore, err := cryptobin_pkcs12.EncodeTrustStore(rand.Reader, certificates, "123")
    if err != nil {
        return err
    }

    pkcs12FileTrustStore := fmt.Sprintf(path, "pkcs12-TrustStore.p12")
    fs.Put(pkcs12FileTrustStore, string(pfxDataTrustStore))

    // 解析测试
    p12, _ := fs.Get(pkcs12File)
    priv, cert, caCerts, err := cryptobin_pkcs12.DecodeChain([]byte(p12), "")
    if err != nil {
        fmt.Println("err =====")
        fmt.Println(err.Error())
        fmt.Println("")
    }

    fmt.Println("priv =====")
    fmt.Printf("%#v", priv)
    fmt.Println("")

    fmt.Println("cert =====")
    fmt.Printf("%#v", cert)
    fmt.Println("")

    fmt.Println("caCerts =====")
    fmt.Printf("%#v", caCerts)
    fmt.Println("")

    fmt.Println("")
    fmt.Println("")

    // 解析测试 DecodeTrustStore
    p12TrustStore, _ := fs.Get(pkcs12FileTrustStore)
    certs, err := cryptobin_pkcs12.DecodeTrustStore([]byte(p12TrustStore), "123")
    if err != nil {
        fmt.Println("TrustStore err =====")
        fmt.Println(err.Error())
        fmt.Println("")
    }

    fmt.Println("TrustStore certs =====")
    fmt.Printf("%#v", certs[0])
    fmt.Println("")

    return nil
}

func MakePKCS12Web() error {
    fs := filesystem.New()

    path := "./runtime/key/pkcs12/assets/%s"

    pkcs12File := fmt.Sprintf(path, "key.p12")
    p12, _ := fs.Get(pkcs12File)

    // 解析测试
    priv, cert, caCerts, err := cryptobin_pkcs12.DecodeChain([]byte(p12), "notasecret")
    if err != nil {
        fmt.Println("err =====")
        fmt.Println(err.Error())
        fmt.Println("")
    }

    fmt.Println("priv =====")
    fmt.Printf("%#v", priv)
    fmt.Println("")

    fmt.Println("cert =====")
    fmt.Printf("%#v", cert)
    fmt.Println("")

    fmt.Println("caCerts =====")
    fmt.Printf("%#v", caCerts)
    fmt.Println("")

    return nil
}

func MakePKCS12Web2() error {
    fs := filesystem.New()

    path := "./runtime/key/pkcs12/p12/%s"

    caCertFile := fmt.Sprintf(path, "ca.der")
    certificateFile := fmt.Sprintf(path, "clientcert.der")
    privateKeyFile := fmt.Sprintf(path, "clientkey.der")

    caCertData, _ := fs.Get(caCertFile)
    certificateData, _ := fs.Get(certificateFile)
    privateKeyData, _ := fs.Get(privateKeyFile)

    caCerts, err := x509.ParseCertificates([]byte(caCertData))
    if err != nil {
        return err
    }

    certificates, err := x509.ParseCertificates([]byte(certificateData))
    if err != nil {
        return err
    }

    var parsedKey any
    if parsedKey, err = x509.ParsePKCS1PrivateKey([]byte(privateKeyData)); err != nil {
        if parsedKey, err = x509.ParsePKCS8PrivateKey([]byte(privateKeyData)); err != nil {
            return err
        }
    }

    var privateKey *rsa.PrivateKey
    var ok bool
    if privateKey, ok = parsedKey.(*rsa.PrivateKey); !ok {
        return errors.New("Rsa error")
    }

    // pfxData, err := cryptobin_pkcs12.Encode(rand.Reader, privateKey, certificates[0], caCerts, "123")
    pfxData, err := cryptobin_pkcs12.Encode(rand.Reader, privateKey, certificates[0], caCerts, "", cryptobin_pkcs12.Opts{
        PKCS8Cipher: cryptobin_pkcs12.GetPKCS8PbeCipherFromName("SHA1AndRC2_40"),
        Cipher: cryptobin_pkcs12.CipherSHA1AndRC2_40,
        KDFOpts: cryptobin_pkcs12.MacOpts{
            SaltSize: 8,
            IterationCount: 1,
            HMACHash: cryptobin_pkcs12.SHA1,
        },
    })
    if err != nil {
        return err
    }

    pkcs12File := fmt.Sprintf(path, "pkcs12.p12")
    fs.Put(pkcs12File, string(pfxData))

    // 保存为 pem 证书
    pkcs12Blocks, err := cryptobin_pkcs12.ToPEM(pfxData, "")
    if err != nil {
        return err
    }

    for k, block := range pkcs12Blocks {
        pkcs12BlockKeyData := pem.EncodeToMemory(block)

        pkcs12BlockFile := fmt.Sprintf(path, fmt.Sprintf("pkcs12-block-%d.key", k))
        fs.Put(pkcs12BlockFile, string(pkcs12BlockKeyData))
    }

    // EncodeTrustStore
    pfxDataTrustStore, err := cryptobin_pkcs12.EncodeTrustStore(rand.Reader, certificates, "123")
    if err != nil {
        return err
    }

    pkcs12FileTrustStore := fmt.Sprintf(path, "pkcs12-TrustStore.p12")
    fs.Put(pkcs12FileTrustStore, string(pfxDataTrustStore))

    // 解析测试
    p12, _ := fs.Get(pkcs12File)
    priv, cert, caCerts, err := cryptobin_pkcs12.DecodeChain([]byte(p12), "")
    if err != nil {
        fmt.Println("err =====")
        fmt.Println(err.Error())
        fmt.Println("")
    }

    fmt.Println("priv =====")
    fmt.Printf("%#v", priv)
    fmt.Println("")

    fmt.Println("cert =====")
    fmt.Printf("%#v", cert)
    fmt.Println("")

    fmt.Println("caCerts =====")
    fmt.Printf("%#v", caCerts)
    fmt.Println("")

    fmt.Println("")
    fmt.Println("")

    // 解析测试 DecodeTrustStore
    p12TrustStore, _ := fs.Get(pkcs12FileTrustStore)
    certs, err := cryptobin_pkcs12.DecodeTrustStore([]byte(p12TrustStore), "123")
    if err != nil {
        fmt.Println("TrustStore err =====")
        fmt.Println(err.Error())
        fmt.Println("")
    }

    fmt.Println("TrustStore certs =====")
    fmt.Printf("%#v", certs[0])
    fmt.Println("")

    return nil
}

var (
    oidHMACWithMD5 = asn1.ObjectIdentifier{1, 2, 840, 113549, 2, 6}
)

// pbkdf2 数据
type pbkdf2Params struct {
    Salt           []byte
    IterationCount int
    PrfParam       pkix.AlgorithmIdentifier `asn1:"optional"`
}

// pbkdf2 数据
type pbkdf2Params2 struct {
    Salt           []byte
    IterationCount int
    KeyLength      int `asn1:"optional"`
    PrfParam       pkix.AlgorithmIdentifier `asn1:"optional"`
}

func TestPKCS12() {
    prfParam := pkix.AlgorithmIdentifier{
        Algorithm:  oidHMACWithMD5,
        Parameters: asn1.RawValue{
            Tag: asn1.TagNull,
        },
    }

    params1 := pbkdf2Params{
        Salt: []byte("asdfr43d"),
        IterationCount: 2048,
        PrfParam: prfParam,
    }
    params2 := pbkdf2Params2{
        Salt: []byte("asdfr43d"),
        IterationCount: 2048,
        KeyLength: 8,
        PrfParam: prfParam,
    }

    marshalledParams1, _ := asn1.Marshal(params1)
    marshalledParams2, _ := asn1.Marshal(params2)

    var params1_1 pbkdf2Params
    if _, err := asn1.Unmarshal(marshalledParams1, &params1_1); err != nil {
        fmt.Println("pbkdf2Params: error params1_1")
    }
    fmt.Print("params1_1: ")
    fmt.Println(params1_1)

    var params1_2 pbkdf2Params
    if _, err := asn1.Unmarshal(marshalledParams2, &params1_2); err != nil {
        fmt.Println("pbkdf2Params: error params1_2")
    }
    fmt.Print("params1_2: ")
    fmt.Println(params1_2)

    var params2_1 pbkdf2Params2
    if _, err := asn1.Unmarshal(marshalledParams1, &params2_1); err != nil {
        fmt.Println("pbkdf2Params2: error params2_1")
    }
    fmt.Print("params2_1: ")
    fmt.Println(params2_1)

    var params2_2 pbkdf2Params2
    if _, err := asn1.Unmarshal(marshalledParams2, &params2_2); err != nil {
        fmt.Println("pbkdf2Params2: error params2_2")
    }
    fmt.Print("params2_2: ")
    fmt.Println(params2_2)
}


var testdata = map[string]string{
    // 'null' password test case
    "Windows Azure Tools": ``,
    // empty string password test case
    "testing@example.com": `
HQ8CAggA`,
}
