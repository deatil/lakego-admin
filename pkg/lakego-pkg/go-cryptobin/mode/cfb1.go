...
func (x *cfb1) XORKeyStream(dst, src []byte) {
    if len(dst) < len(src) {
        panic("go-cryptobin/cfb1: output smaller than input")
    }
    if alias.InexactOverlap(dst[:len(src)], src) {
        panic("go-cryptobin/cfb1: invalid buffer overlap")
    }
...
func NewCFB1(block cipher.Block, iv []byte, decrypt bool) cipher.Stream {
    ...
    if len(iv) != blockSize {
        panic("go-cryptobin/cfb1: IV length must equal block size")
    }
