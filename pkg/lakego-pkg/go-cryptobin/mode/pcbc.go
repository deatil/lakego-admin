...
func NewPCBCEncrypter(b cipher.Block, iv []byte) cipher.BlockMode {
    if len(iv) != b.BlockSize() {
        panic("go-cryptobin/pcbc.NewPCBCEncrypter: IV length must equal block size")
    }
...
func newPCBCGenericEncrypter(b cipher.Block, iv []byte) cipher.BlockMode {
    if len(iv) != b.BlockSize() {
        panic("go-cryptobin/pcbc: IV length must equal block size")
    }
    return (*pcbcEncrypter)(newPCBC(b, iv))
}
...
func (x *pcbcEncrypter) CryptBlocks(dst, src []byte) {
    if len(src)%x.blockSize != 0 {
        panic("go-cryptobin/pcbc: input not full blocks")
    }

    if len(dst) < len(src) {
        panic("go-cryptobin/pcbc: output smaller than input")
    }

    if alias.InexactOverlap(dst[:len(src)], src) {
        panic("go-cryptobin/pcbc: invalid buffer overlap")
    }
...
func (x *pcbcEncrypter) SetIV(iv []byte) {
    if len(iv) != len(x.iv) {
        panic("go-cryptobin/pcbc: incorrect length IV")
    }
...
func NewPCBCDecrypter(b cipher.Block, iv []byte) cipher.BlockMode {
    if len(iv) != b.BlockSize() {
        panic("go-cryptobin/pcbc.NewPCBCDecrypter: IV length must equal block size")
    }
