...
func Test_Check(t *testing.T) {
    ...
    for i = 0; i < 3; i++ {
        zuc := NewZucState(key[i][:], iv[i][:])

        var buf [3]uint32
        zuc.GenerateKeystream(2, buf[:])

        if (buf[0] != ciphertext[i][0] || buf[1] != ciphertext[i][1]) {
            t.Errorf("error generating ZUC key stream on test vector %d\n", i)
        }
    }
}

/* test vector from GM/T 0001.2-2012 */
func Test_Eea_Check(t *testing.T) {
    var key = [][16]byte{
    ...
    var count = []uint32{0x66035492, 0x56823, 0x2738cdaa}
    var bearer = []uint32{0x0f, 0x18, 0x1a}
    var direction = []uint32{0, 1, 0}
    var ibs0 = []uint32{
    ...
    var ibs = [][]uint32{ibs0, ibs1, ibs2};
    var obs = [][]uint32{obs0, obs1, obs2};
    var bits = []int{0xc1, 0x320, 0xfb3};

    nn := len(obs2)/4

    var buf []uint32 = make([]uint32, nn)
    var i int

    for i = 0; i < len(key)/len(key[i]); i++ {
        EeaEncrypt(ibs[i][:], buf[:], bits[i], key[i][:], count[i], bearer[i], direction[i])

        n := EEA_ENCRYPT_NBYTES(bits[i])

        if !reflect.DeepEqual(buf[:n], obs[i][:n]) {
            t.Errorf("zuc eea test %d failed\n", i);
        }
    }
}

func bswap_buf(buf []uint32, nwords int) {
    var i int
...
func Test_Eia_Check(t *testing.T) {
    var key = [][16]byte{
    ...
    var count = []uint32{0, 0xa94059da, 0x561eb2dd}
    var bearer = []uint32{0, 0x0a, 0x1c}
    var direction = []uint32{0, 1, 0}
    var mesg0 = []uint32{0}
    var mesg1 = []uint32{
    ...
    var mesg = [][]uint32{mesg0, mesg1, mesg2}
    var bits = []int{1, 0x241, 0x1626}
    var mac = []uint32{0xc8a9595e, 0xfae8ff0b, 0x0ca12792}
    var i int

    bswap_buf(mesg0, len(mesg0)/4)
    bswap_buf(mesg1, len(mesg1)/4)
    bswap_buf(mesg2, len(mesg2)/4)

    for i = 0; i < len(key)/len(key[0]); i++ {
        var T uint32

        T = EiaGenerateMac(mesg[i], bits[i], key[i], count[i], bearer[i], direction[i])

        if (T != mac[i]) {
            t.Errorf("zuc eia test %d failed\n", i)
        }
    }
}

/* from ZUC256 draft */
func Test_Zuc256_Check(t *testing.T) {
    var key = [][32]byte{
        {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
         0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
         0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
    ...
    for i = 0; i < len(key)/4; i++ {
        ...
        zuc_key := NewZuc256State(key[i][:], iv[i][:])
        zuc_key.GenerateKeystream(20, buf[:])

        if !reflect.DeepEqual(buf[:], ciphertext[i][:20]) {
            t.Errorf("zuc256 test %d failed\n", i);
        }
    }
}

func Test_Zuc256_Mac_Check(t *testing.T) {
    var key = [][32]byte{
        {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
         0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
         0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
    ...
    var tag64 = []uint32{
        0x99543e67,0x8cd33400,
        0x130dc225,0xe72240cc,
        0x8c71394d,0x39957725,
        0xea1dee54,0x4bb6223b,
    };
    var tag128 = []uint32{
        0xbb545ed8,0x960096cb,0x52c98470,0x264b65a1,
    ...
    bswap_buf(tag32, len(tag32)/4)
    bswap_buf(tag64, len(tag64)/4)
    bswap_buf(tag128, len(tag128)/4)

    for i = 0; i < 4; i++ {
        var ctx *Zuc256Mac
        var mac []byte

        ctx = NewZuc256Mac(key[i][:], iv[i][:], 32)
        ...
        macs := uint32sToBytes(tag32[i:i+1])

        if !reflect.DeepEqual(mac[:4], macs) {
            t.Errorf("zuc256 mac test %d 32-bit failed\n", i)
        }

        // =========

        ctx = NewZuc256Mac(key[i][:], iv[i][:], 64)
        ...
        macs = uint32sToBytes(tag64[i*2:i*2+2])

        if !reflect.DeepEqual(mac[:8], macs) {
            t.Errorf("zuc256 mac test %d 64-bit failed\n", i);
        }

        // =========

        ctx = NewZuc256Mac(key[i][:], iv[i][:], 128)
        ...
        macs = uint32sToBytes(tag128[i*4:i*4+4])

        if !reflect.DeepEqual(mac[:16], macs) {
            t.Errorf("zuc256 mac test %d 128-bit failed\n", i);
        }
    }
}
