...
func Test_ELGamal(t *testing.T) {
    var testBitsize = 256
    var testProbability = 64

    priv, err := elgamal.GenerateKey(rand.Reader, testBitsize, testProbability) // 生成密钥对
    if err != nil {
        t.Fatal(err)
    }

    privPem, err := elgamal.MarshalPKCS8PrivateKey(priv) // 生成密钥文件
    if err != nil {
        t.Fatal(err)
    }

    privKey, err := elgamal.ParsePKCS8PrivateKey(privPem) // 读取密钥
    if err != nil {
        t.Fatal(err)
    }
    ...
    pubKey, _ := priv.Public().(*elgamal.PublicKey)
    pubkeyPem, err := elgamal.MarshalPKCS8PublicKey(pubKey)       // 生成公钥文件

    pubKey2, err := elgamal.ParsePKCS8PublicKey(pubkeyPem) // 读取公钥
    if err != nil {
        t.Fatal(err)
    }
...
func Test_ELGamal2(t *testing.T) {
    var testBitsize = 256
    var testProbability = 64
