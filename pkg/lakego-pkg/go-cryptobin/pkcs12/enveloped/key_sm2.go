            ...
            switch k.Curve {
                case sm2.P256():
                    ...
                    if !k.IsOnCurve(k.X, k.Y) {
                        return nil, errors.New("go-cryptobin/pkcs12: error while validating SM2 public key: %v")
                    }
            }
        default:
            return nil, errors.New("go-cryptobin/pkcs12: PublicKey is not sm2 PublicKey")
    }
...
func (this KeyEncryptWithSM2) Decrypt(ciphertext []byte, pkey crypto.PrivateKey) ([]byte, error) {
    ...
    if priv, ok = pkey.(*sm2.PrivateKey); !ok {
        return nil, errors.New("go-cryptobin/pkcs12: PrivateKey is not sm2 PrivateKey")
    }
