...
func Test_AddNamedCurve(t *testing.T) {
    assertNoError := cryptobin_test.AssertNoErrorT(t)
    assertEqual := cryptobin_test.AssertEqualT(t)
    assertNotEmpty := cryptobin_test.AssertNotEmptyT(t)

    curveGostR34102001ParamSetccTest := func() *gost.Curve {
        curve, _ := gost.NewCurve(
        ...
        curve.Name = "GostR34102001ParamSetccTest"
        return curve
    }
    oidGostR3410_2001_ParamSet_Test := asn1.ObjectIdentifier{1, 2, 643, 2, 2, 35, 999}

    AddNamedCurve(curveGostR34102001ParamSetccTest(), oidGostR3410_2001_ParamSet_Test)

    obj := New().WithCurve(curveGostR34102001ParamSetccTest()).GenerateKey()
    assertNoError(obj.Error(), "PrivateKeyD")

    priv := obj.GetPrivateKeyBytes()
    assertNotEmpty(priv, "PrivateKey_Bytes")

    xk := New().WithCurve(curveGostR34102001ParamSetccTest()).FromPrivateKeyBytes(priv)
