...
func MarshalECPrivateKey(key *PrivateKey) ([]byte, error) {
    ...
    if !ok {
        return nil, errors.New("go-cryptobin/eckcdsa: unknown elliptic curve")
    }
...
func marshalECPrivateKeyWithOID(key *PrivateKey, oid asn1.ObjectIdentifier) ([]byte, error) {
    if !key.Curve.IsOnCurve(key.X, key.Y) {
        return nil, errors.New("go-cryptobin/eckcdsa: invalid elliptic key public key")
    }
...
func parseECPrivateKey(namedCurveOID *asn1.ObjectIdentifier, der []byte) (key *PrivateKey, err error) {
    ...
    if _, err := asn1.Unmarshal(der, &privKey); err != nil {
        return nil, errors.New("go-cryptobin/eckcdsa: failed to parse EC private key: " + err.Error())
    }

    if privKey.Version != ecPrivKeyVersion {
        return nil, fmt.Errorf("go-cryptobin/eckcdsa: unknown EC private key version %d", privKey.Version)
    }
    ...
    if k.Cmp(curveOrder) >= 0 {
        return nil, errors.New("go-cryptobin/eckcdsa: invalid elliptic curve private key value")
    }
    ...
    for len(privKey.PrivateKey) > len(privateKey) {
        if privKey.PrivateKey[0] != 0 {
            return nil, errors.New("go-cryptobin/eckcdsa: invalid private key length")
        }
