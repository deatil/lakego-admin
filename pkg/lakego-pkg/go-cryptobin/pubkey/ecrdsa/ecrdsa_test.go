...
func Test_SignVerify(t *testing.T) {
    t.Run("true", func(t *testing.T) {
        test_SignVerify(t, true)
    })
    t.Run("false", func(t *testing.T) {
        test_SignVerify(t, false)
    })
}

func test_SignVerify(t *testing.T, useISO14888_3 bool) {
    priv, err := GenerateKey(rand.Reader, elliptic.P224())
    if err != nil {
        t.Fatal(err)
    }
    ...
    sig, err := Sign(rand.Reader, priv, sha256.New, data, useISO14888_3)
    if err != nil {
        t.Fatal(err)
    }

    res := Verify(pub, sha256.New, data, sig, useISO14888_3)
    if !res {
        t.Error("Verify fail")
    }
...
func Test_SignBytes(t *testing.T) {
    t.Run("P224 sha256", func(t *testing.T) {
        test_SignBytes(t, elliptic.P224(), sha256.New, true)
    })
    t.Run("P256 sha256", func(t *testing.T) {
        test_SignBytes(t, elliptic.P256(), sha256.New, true)
    })

    t.Run("P224 sha256 false", func(t *testing.T) {
        test_SignBytes(t, elliptic.P224(), sha256.New, false)
    })
    t.Run("P256 sha256 false", func(t *testing.T) {
        test_SignBytes(t, elliptic.P256(), sha256.New, false)
    })
}

func test_SignBytes(t *testing.T, c elliptic.Curve, h Hasher, useISO14888_3 bool) {
    ...
    sig, err := SignBytes(rand.Reader, priv, h, data, useISO14888_3)
    if err != nil {
        t.Fatal(err)
    }

    res := VerifyBytes(pub, h, data, sig, useISO14888_3)
    if !res {
        t.Error("Verify fail")
    }
