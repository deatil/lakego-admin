...
func newLyra2RE() *lyra2re {
    l := &lyra2re{}
    l.state = make([]uint64, 16)

    copy(l.state[8:], blake2bIV)
...
func (this *lyra2re) squeeze(out []byte) {
    ...
    for j := 0; j < len(out)/blockLenBytes+1; j++ {
        for i := 0; i < blockLenInt64; i++ {
            binary.LittleEndian.PutUint64(tmp[i*8:], this.state[i])
        }

        // be care in case of len(out[i:])<len(tmp)
        copy(out[j*blockLenBytes:], tmp)
        blake2bLyra(this.state)
    }
}
 ...
 */
func (this *lyra2re) reducedSqueezeRow0(rowOut []uint64, nCols int) {
 ...
 * @param nCols  Number of columns of the memory matrix (C)
 */
func (this *lyra2re) reducedDuplexRow1(
    rowIn  []uint64,
    rowOut []uint64,
    nCols  int,
) {
    ...
    for i := 0; i < nCols; i++ {
        ...
        for j = 0; j < 12; j++ {
            ptrWordOut[j] = ptrWordIn[j] ^ this.state[j]
        }
 ...
 * @param nCols    Number of columns of the memory matrix (C)
 */
func (this *lyra2re) reducedDuplexRowSetup(
    rowIn    []uint64,
    rowInOut []uint64,
    rowOut   []uint64,
    nCols    int,
    ...
    for i := 0; i < nCols; i++ {
        ...
        newState := append(this.state[11:12], this.state[:11]...)
        for j = 0; j < 12; j++ {
 ...
 * @param nCols    Number of columns of the memory matrix (C)
 */
func (this *lyra2re) reducedDuplexRow(
    rowIn    []uint64,
    rowInOut []uint64,
    rowOut   []uint64,
    nCols    int,
...
func Lyra2(
    ...
    state.reducedSqueezeRow0(memMatrix[0], nCols) //The locally copied password is most likely overwritten here
    state.reducedDuplexRow1(memMatrix[0], memMatrix[1], nCols)

    for row < nRows {
        //M[row] = rand; //M[row*] = M[row*] XOR rotW(rand)
        state.reducedDuplexRowSetup(memMatrix[prev], memMatrix[rowa], memMatrix[row], nCols)
    ...
    for tau = 1; tau <= timeCost; tau++ {
        ...
        for row0 := false; !row0; row0 = (row == 0) {
            ...
            rowa = state.state[0] % uint64(nRows) //(USE THIS FOR THE "GENERIC" CASE)

            //Performs a reduced-round duplexing operation over M[row*] XOR M[prev], updating both M[row*] and M[row]
            state.reducedDuplexRow(memMatrix[prev], memMatrix[rowa], memMatrix[row], nCols)
