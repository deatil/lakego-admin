...
func (d *digest) msgExpansion(in []byte) {
    for i := 0; i < 32; i++ {
        d.msg[i] = binary.LittleEndian.Uint64(in[i*8:])
    }

    for i := 2; i <= numStep; i++ {
        idx := 16 * i
        d.msg[idx+0] = d.msg[idx-16] + d.msg[idx-29]
...
func (d *digest) step(stepidx, alpha, beta int) {
    ...
    for colidx := 0; colidx < 8; colidx++ {
        ...
        d.tcv[colidx] = vr + vl
        d.tcv[colidx+8] = bits.RotateLeft64(vr, gamma[colidx])
    }
